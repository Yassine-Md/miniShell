...
struct cmdline *readcmd(void)
{
	...
	// Lecture d'une ligne de texte depuis l'entree utilisateur
	line = readline();

	// Si la ligne est nulle, liberation de la mémoire de la commande precedente et retour
	if (line == NULL) {
		if (s) {
			freecmd(s);
			free(s);
	...
	// Allocation et Initialisation des variables de commande et de sequence
	...
	// Allocation de memoire pour stocker les mots de la ligne
	...
	// Initialisation des membres de la structure cmdline
	s->err = 0;
	s->in = 0;
	s->out = 0;
	s->seq = 0;

	// Boucle de traitement des mots de la ligne
	i = 0;
	while ((w = words[i++]) != 0) {
		switch (w[0]) {

		// Gestion de la redirection d'entree
		case '<':
...
// En cas d'erreur, liberation de la memoire allouee et retou
error:
	while ((w = words[i++]) != 0) {
		switch (w[0]) {
		case '<':
